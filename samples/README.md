...
| --- | --- | --- |
| `samples.scalar_values` | `scalar_values_template.xlsx`, `scalar_values_output.xlsx` | Native scalar cells, mixed text, mapping access, native dates versus the textual `date` filter, booleans, numeric and record-column `sum`, `upper`, `join`, and `default` filters |
